...
exports.create = (medicoData, callback) => {
  ...
  const query = `
    INSERT INTO medicos (username, password, crm, nascimento, email, celular, cpf, cep, numero, bairro, cidade, estado)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `;
...
exports.update = (id, medicoData, callback) => {
  ...
  const query = `
    UPDATE medicos SET username = ?, password = ?, crm = ?, nascimento = ?, email = ?, celular = ?, cpf = ?, cep = ?, numero = ?, bairro = ?, cidade = ?, estado = ?
    WHERE id = ?
  `;
